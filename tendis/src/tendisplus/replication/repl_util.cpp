...
Expected<BinlogResult> applySingleTxnV2(Session* sess,
  ...
  auto value = ReplLogValueV2::decode(logValue);  //传入的logValue解析到value
  if (!value.ok()) {
    return value.status();
  }
  ...
  while (offset < dataSize) {   //1、处理binlog格式的数据写入DB
    size_t size = 0;
...
//2、以下准备写binlog到DB
  ...
  if (mode == BinlogApplyMode::KEEP_BINLOG_ID) {
    ...
    auto s = txn->setBinlogKV(binlogId, logKey, logValue);  //2.1、主从复制时直接保存，使用master的binlogid
    if (!s.ok()) {
      return s;
    }
  } else {  // migrating chunk.
    // use self binlogId to replace sender's binlogId
    auto s = txn->setBinlogKV(logKey, logValue);  //2.2、扩缩容migrate来的数据的数据则使用自己的binlogid保存
    if (!s.ok()) {
      return s;
    }
