...
Expected<uint64_t> RocksTxn::commit() {
  ...
  const auto guard = MakeGuard([this, &binlogTxnId] {   //构造锁，锁事务，保证数据提交与binlog提交原子性
    _txn.reset();
  ...
  if (_replLogValues.size() != 0) {
    ...
    auto s = _txn->Put(_store->getBinlogColumnFamilyHandle(), //binlog写入binlog的CF中
                       key.encode(),
  ...
  auto s = _txn->Commit();    //统一提交写入的key-value、以及binlog的内容;rocksdb::PessimisticTransaction::Commit
  if (s.ok()) {
    return _txnId;
  } else {
    binlogTxnId = Transaction::TXNID_UNINITED;
...
Status RocksTxn::setKV(const std::string& key,
  ...
  auto s = _txn->Put(key, val); //Data Put到默认的CF中，使用rocksdb的transaction来保证操作的同步
  if (!s.ok()) {
    return {ErrorCodes::ERR_INTERNAL, s.ToString()};
  }
  ...
  if (_store->enableRepllog()) {
    ...
    if (_replLogValues.size() == std::numeric_limits<uint16_t>::max()) {  //_replLogValues元素就是ReplLogValueEntryV2格式的binlog内容
    ...
    ReplLogValueEntryV2 logVal(   //构造binlog记录ReplLogValueEntryV2格式
      ReplOp::REPL_OP_SET, ts ? ts : msSinceEpoch(), key, val);
    // TODO(vinchen): maybe OOM
    _replLogValues.emplace_back(std::move(logVal)); //put到_replLogValues
  }
  return {ErrorCodes::ERR_OK, ""};
}
...
Status RocksTxn::applyBinlog(const ReplLogValueEntryV2& logEntry) {
  ...
  switch (logEntry.getOp()) {
    case ReplOp::REPL_OP_SET: {   //ReplOp::REPL_OP_SET在setKV生成binlog记录时候设置的
